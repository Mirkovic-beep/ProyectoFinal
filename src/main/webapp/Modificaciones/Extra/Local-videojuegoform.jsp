...
ArrayList<Local> localeslist = controladorBD.dameLocales();
...
locales = controladorBD.dameLocalVideojuego(id_videojuego);
localesold = controladorBD.dameLocalVideojuego(id_videojuego);
%>
			...
			<div class="breadcrumbs">
				<div class="container">
					...
					<a href="../../modificaciones.jsp">Modificaciones</a> <a
						...
						href="Local-videojuegoform.jsp">Compras</a> <span>Form</span>
				</div>
			</div>
			...
			<div class="page">
				...
				<div class="product-list">
					...
					<div class="login-box">
						...
						<h2>Modificar local asociado videojuego</h2>

						<form
							action="../../operaciones_mod.jsp?accion=ModificarVideojuegoLocal"
							method="post">

							<div class="mm-dropdown">
								<div class="textfirst">Locales disponibles</div>
								...
								<ul class="scrollable-menu">
									<li class="input-option"
										data-value="<%=localeslist.get(i).getNombre()%>">
										<h3 id="centro"><%=localeslist.get(i).getNombre()%></h3>
									</li>
								</ul>
							...
							%>
							<div class="user-box">
								<input type="text" name="nombre_local<%=i%>" required
									placeholder="Número de copias" value="<%=locales.get(i)%>"
									data-bs-toggle="tooltip" data-bs-html="true"
									data-bs-placement="right"
									title="<%for (int j = 0; j < localeslist.size(); j++) {%> <%=localeslist.get(j).getNombre()%><%}%>" />
							...
							%>
							<input type="hidden"
								value="<%=controladorBD.dameCodigoLocalNombre(locales.get(i))%>"
								name="id_local<%=i%>" id="id_local<%=i%>">
							<%
							}
							%>

							<%
							for (int i = 0; i < localesold.size(); i++) {
							%>
							<input type="hidden" value="<%=localesold.get(i)%>"
								name="nombre_localesold<%=i%>" id="nombre_localesold<%=i%>">
							<%
							}
							%>

							<input type="hidden" value="<%=locales.size()%>"
								name="count_locales" id="count_locales"> <br> <br>
